...
from aircraft_heading_gnn.data.dataset import AircraftGraphDataset, create_train_val_test_split
...
def evaluate(model, loader, criterion, device):
    ...
    if all_preds:
        acc_15deg = compute_heading_accuracy(np.array(all_preds), np.array(all_targets), 15)
        acc_30deg = compute_heading_accuracy(np.array(all_preds), np.array(all_targets), 30)
    else:
...
def main():
    ...
    # Check data quality
    sample = dataset[0]
    print(f"Sample graph: {sample.x.shape[0]} nodes, {sample.edge_index.shape[1]//2} edges")
    ...
    for epoch in range(args.epochs):
        ...
        # Validate
        val_loss, val_acc_15, val_acc_30, val_samples = evaluate(model, val_loader, criterion, device)
    ...
    test_loss, test_acc_15, test_acc_30, test_samples = evaluate(model, test_loader, criterion, device)
