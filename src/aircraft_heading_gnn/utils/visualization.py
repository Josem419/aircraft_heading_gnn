...
def plot_trajectories(
    ...
    for traj_id in unique_trajs:
        ...
        if color_by == "velocity" and "velocity" in df.columns:
            scatter = ax.scatter(
                traj_data["lon"],
                traj_data["lat"],
                c=traj_data["velocity"],
                cmap="viridis",
                s=10,
                alpha=0.6,
            )
        else:
            ax.plot(traj_data["lon"], traj_data["lat"], alpha=0.5, linewidth=1)
    ...
    if airport_lat is not None and airport_lon is not None:
        ax.plot(
            ...
            "r*",
            markersize=20,
            label=f"Airport: {airport_icao or ''}",
        )
...
def plot_graph_snapshot(
    ...
    if hasattr(graph_data, "has_label"):
        colors = [
            "green" if has_lbl else "red" for has_lbl in graph_data.has_label.numpy()
        ]
    else:
        colors = "blue"
    ...
    # Plot airport
    if airport_lat is not None and airport_lon is not None:
        ax.plot(airport_lon, airport_lat, "r*", markersize=20, label=f"Airport ({airport_icao})")

    ax.set_xlabel("Longitude (degrees)")
    ax.set_ylabel("Latitude (degrees)")
    ax.set_title(
        f"Graph Snapshot (Nodes: {len(lats)}, Edges: {graph_data.edge_index.shape[1]})"
    )
    ax.grid(True, alpha=0.3)
    ax.legend()
